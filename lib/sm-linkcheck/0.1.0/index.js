...
export async function checkLinks(page, options = {}) {
  ...
  if (links.length === 0) {
    check(null, { 'sm-linkcheck: no broken links found': () => true })
    return { url, checked: 0, broken: [], results: [] }
  }

  const responses = http.batch(
    links.map((link) => ({ method, url: link, params: { timeout, ...params } }))
  )

  // Some servers don't support HEAD; retry those with GET before giving up.
  if (method === 'HEAD') {
    const retryIndexes = []
    responses.forEach((res, i) => {
      if (res.status === 405) retryIndexes.push(i)
    })

    if (retryIndexes.length > 0) {
      const retried = http.batch(
        retryIndexes.map((i) => ({ method: 'GET', url: links[i], params: { timeout, ...params } }))
      )
      retryIndexes.forEach((i, j) => {
