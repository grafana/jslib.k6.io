...
function humanizeGenericDuration(dur) {
    ...
    if (dur < 1000) { // duration is smaller than a second
    ...
    if (rem < 1) { // less than a minute
        return result;
    }
    result = (rem % 60) + 'm' + result;
    rem = Math.trunc(rem / 60);
    if (rem < 1) { // less than an hour
        return result;
    }
    return rem + 'h' + result;
}
...
function summarizeMetrics(options, data, decorate) {
    ...
    var getData = function (name) {
        if (trendCols.hasOwnProperty(name)) {
            ...
            for (var i = 0; i < cols.length; i++) {
                tmpCols[i] = options.summaryTrendStats[i] + '=' + decorate(cols[i], palette.cyan) +
    ...
    for (var name of names) {
        ...
        var mark = ' ';
        var markColor = function (text) { return text; }; // noop

        if (metric.thresholds) {
            mark = succMark;
            markColor = function (text) { return decorate(text, palette.green); };
            forEach(metric.thresholds, function (name, threshold) {
                if (!threshold.ok) {
                    mark = failMark;
                    markColor = function (text) { return decorate(text, palette.red); };
                    return true; // break
                }
            });
        }
        ...
        fmtName = fmtName + decorate(
            '.'.repeat(nameLenMax - strWidth(fmtName) - strWidth(fmtIndent) + 3) + ':', palette.faint
        );
...
function generateTextSummary(data, options) {
    ...
    var decorate = function (text) { return text; }
...
function escapeHTML(str) {
    ...
    return str.replace(/[&<>'"]/g, function (char) { return replacements[char] });
};
