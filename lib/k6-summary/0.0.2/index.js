 ...
 */
function generateJUnitXMLForChecks(data, suiteName) {
  var failures = 0
  var cases = []
  var time = data.metrics.iteration_duration.values.max
    ? data.metrics.iteration_duration.values.max
    : 0
  suiteName = suiteName ? escapeHTML(suiteName) : 'k6 checks'
  if (data.root_group.hasOwnProperty('checks') && data.root_group.checks.length > 0) {
    ...
    checks.forEach((check) => {
      if (check.passes >= 1 && check.fails === 0) {
        ...
        failures++
        cases.push(
          `<testcase name="${escapeHTML(check.name)}"><failure message="failed"/></testcase>`
        )
      }
    })
  }
  return (
    ...
    `<testsuite name="${suiteName}" tests="${cases.length}" failures="${failures}" ` +
...
exports.jUnitForChecks = generateJUnitXMLForChecks
